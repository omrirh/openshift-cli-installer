...
class OCPCluster:
    def __init__(self, ocp_cluster, user_input):
        ...
        self.s3_bucket_name = self.user_input.s3_bucket_name or (
            self.cluster["cluster_info"].get("s3_bucket_name") if self.cluster.get("cluster_info") else None
        )
        self.s3_bucket_path = self.user_input.s3_bucket_path or (
            self.cluster["cluster_info"].get("s3_bucket_path") if self.cluster.get("cluster_info") else None
        )
